...
This specification provides five Annexes, three of which are to facilitate development. The SLPF schema (Annex A), the applicable portions of the OpenC2 Language schema (Annex B) and multiple examples of SLPF commands (Annex C).   
...
* insert_rule:
    ...
    * The value MUST map to exactly one deny <target> or allow <target> for a given instance of an SLPF.
* directionality: 
...
Case Two: the command failed due to a syntax error in the command.  Optional status text is ignored by the openc2 producer, but may be added to provide details for debugging or logging.
...
Permit ftp data transfers to ip address 198.51.100.17 from any source.  (Note that an actual application would also need to allow ftp-data (port 20) in order for transfers to be permitted.) Also note that the actuator called out was SLPF with no additional specifiers. In a multicast environment, this indicates that all endpoints that can execute the command should. If an endpoint supports multiple profiles, a poplulated actuator field indicates which profile is to be used to process the command.
